...
namespace _11._Array
{
    internal class SAtringClass
    {
        ...
        {
            // string 사용
            string str = "abcde"; //char 5개짜리 배열이라고 생각하면 됨
            ...
            Console.WriteLine(array);  // output : abced
            array.Contains('b'); //배열에 'b'를 포함하냐 t/f
            str.Contains('b'); // 사실상 string 은 문자의 배열이기 때문에 Contains 사용 가능 ( 포함됐냐 안됐냐 )
            ...
            //string은 class이지만 값형식처럼 사용되어야 하기 때문에, 힙영역에 문자열을 복사하여 str2가 참조하도록 함
        ...
        // <문자열 간격 및 형식>
        // 문자열 형식을 표현하는데 값의 간격과 형식을 정함 (,얼마만큼의 간격을 쓸건지 의 형식으로 사용)
        void Main7()
        {
            // 문자열 간격
            ...
            // - 숫자 형식 - ( 넘버링 : 숫자형식으로 나오고 소숫점 둘째자리까지 나옴)
            Console.WriteLine($"{123456789:N2}");           // output : 123,456,789.00

            // - 고정소수점 형식 - (원하는 소수점 자릿수까지 반올림)
            ...
            // 문자열 간격과 형식 (두가지 방식을 섞어서 사용도 가능)
            Console.WriteLine($"Text:{123.456,+8:F2}");    // output : Text:  123.46
        }

        //<StringBuilder> 문자를 여러개 붙여서 사용해야 되는 경우
