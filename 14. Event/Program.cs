namespace _14._Event
{
    internal class Program
    {
         ...
         * 이벤트 (Event)
         * 
         * 일련의 사건이 발생했다는 사실을 다른 객체에게 전달 (공지때리기)
         * 델리게이트의 일부 기능을 제한하여 이벤트의 용도로 사용 - 콜백을 여러대상한테 전달하는 것과 같음
         ****************************************************************/
        ...
        public class Player
        {
            public event Action OnGetCoin;  // 이벤트 : 무조건 이벤트로만 사용 가능
            ...
            public void GetCoin()
            {
                ...
                if (OnGetCoin != null) OnGetCoin(); // 일련의 사건이 발생했을 때 이벤트 발생
            }
            ...
            public void GetDamaged()
            {
                ...
                if (OnDamaged != null) OnDamaged(); // 일련의 사건이 발생했을 때 이벤트 발생
            }
        }
        ...
        void Main1()
        {
            ...
            // <문제점>
            //1. 델리게이트 체인 = (대입)  : 대입이 가능함 (이전 이벤트들 다 사라질 수 있음) --> 이벤트는 대입이 막힘 불가능임
            player.OnGetCoin = sfx.CoinSound; // 대입을 하면 기존의 객체들이 사라져버림 (error)
            //2. 이벤트 발생이 외부에서 가능 : 외부에서 함부로 호출할 수도 있음
            player.OnGetCoin(); // (error)
            player.OnDied(); // 실제로 실행된게 없는데 있는것처럼 소문남
